...
| Embedding | OpenAI `text-embedding-3-small` 起步 | **Anthropic 沒有 embeddings endpoint**——生成與 embedding 是兩個獨立的選擇。第 6 週把本地 BGE-M3 拉進來當實驗項 |
...
| 前端介面 | Streamlit | 聊天介面約 50 行;可部署 Streamlit Cloud(免費)。**第 5 週上線** |
...
- [ ] **跨語言檢查(本週最重要的一項)**:準備幾組互為翻譯的中英句子,看中文句子是否真的
      ...
      現在十個句子就測得完,等到第 6 週才發現就是重跑整條 pipeline。
...
- [ ] **距離閘門的 τ 這時還是暫定值,介面要說出來**:提示綁在 `config.py` 的
      `provisional` 旗標上,第 6 週推導完翻掉旗標,提示就自己消失。一個說不出自己
      沒校準過的閘門,使用者只會把 abstention 讀成「我的筆記沒寫」
- [ ] (選配)上傳新文件即時入庫
- **里程碑**:可以 demo 的網頁版,並且開始拿它讀自己的筆記——答得爛的實例記下來,
  那就是第 6 週測試集的素材
...
**評測排在前端之後**,代價要講清楚:介面在第 5 週就上線,而 τ 要到這一週才推導出來,
所以中間那段時間畫面跑的是 `config.py` 裡 `provisional=True` 的暫定值。換來的是工具早一週
真的被用起來——而且第 5 週手上那些「這題答得爛」的實例,正好是這一週出測試集的素材,
比憑空想 20 題可答題便宜得多。這一週把 τ 推導完、把 `provisional` 翻成 `False`,
介面上那句「τ 尚未推導」就會自己消失。
...
- [ ] 掃 τ 並套用選點規則:**在 false abstention ≤ 5%(20 題最多錯 1 題)的前提下取最大的 τ**
      ...
      並確認 CLI 與介面上的「暫定 τ」提示都自動消失了——提示綁在那個旗標上,
      不該需要第二次修改
- [ ] 實驗 → **記錄成表**。只剩一條 chunking 路徑,但切片的紀律照舊——切片欄位現在就留著,
  ...
  | `window` / `overlap` | 非結構化 | 路徑已隨 #7 落地(ADR-0007),`WINDOW_TOKENS` / `WINDOW_OVERLAP_TOKENS` 待掃 |
...
| 項目 | 主線的話會落在 | 為什麼一起走 |
|---|---|---|
| ~~`ingest_papers.py`~~ | 第 3 週 | **已完成(#7)**——沒有這支腳本。ADR-0006 依格式分流,論文跟筆記走同一條 `cli.py ingest` |
| ~~非結構化 chunking(固定長度 + 重疊)~~ | 第 4 週 | **已完成(#7)**:`chunk_windows`,參數在 `config.py`(ADR-0007) |
| ~~PDF 筆記~~ | 第 3 週 | **已完成(#7)**:`.pdf` 進入走訪,依格式拿到 Passage 就走窗格路徑 |
| `ingest_wiki.py` / `ingest_articles.py` | 第 3 週 | 兩支腳本本身;文章(#8)已可沿用窗格路徑 |
| 評測的「跨來源題 6 題」 | 第 6 週 | 只有一種 `source_type` 時題目不成立 |
| `window` / `overlap` 參數掃描 | 第 6 週 | 掃的是非結構化路徑的參數——現在有樣本可切了 |
| 衝突偵測 | 第 7 週(進階優化) | 「不同來源說法不一致」需要第二種來源才比得出來 |
| 側邊欄的來源類型篩選 | 第 5 週(介面) | 單一來源時是個只有一個選項的選單 |
...
> **論文(#7)接回來時,這四件事只走完第 2 件的一半。** 路徑與 locator 產生方式
> 已落地(ADR-0007),`window` / `overlap` 進了 `config.py` 等第 6 週掃;第 1 件的
> **人工抽查**還沒做過——測試用的 PDF 是測試時自己寫的,不是真的雙欄論文,
> 所以雙欄抽取的順序目前沒有任何證據支撐,第一次匯入真論文時必須人工看過。
> 第 3、4 件要等評測儀器,英文論文進來時 τ 也要重看。
...
| 檢索品質差 | 答非所問 | 回到第 6 週評測集,依 chunking 路徑切片後逐項實驗,不要瞎調 |
